...
func TestLoginUsersController(t *testing.T) {
	var testCases = []struct {
	...
	}{
		{
			...
			reqBody:       `{"email": "alta@gmail.com", "password": "123"}`,
			expectCode:    http.StatusOK,
			expectMessage: "success login",
		},
		{
			name:          "incorrect email or password",
			reqBody:       `{"email": "aw@test.com", "password": "1111"}`,
			expectCode:    http.StatusBadRequest,
			expectMessage: "failed to login",
		},
	}
	...
	for _, tc := range testCases {
		req := httptest.NewRequest(http.MethodPost, "/login", bytes.NewBuffer([]byte(tc.reqBody)))
		req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
		res := httptest.NewRecorder()
		c := e.NewContext(req, res)
